...
import me.jesusmx.hubcore.util.tablist.shared.entry.TabEntry;
...
public abstract class TabAdapter {
    ...
    public TabAdapter handleElement(Player player, TabElement element) {
        int rows = this.getMaxElements(player) / 20;

        for (int y = 0; y < 20; y++) {
            for (int x = 0; x < rows; x++) {
                TabEntry entry = element.getEntry(x, y);
                int index = y * rows + x;
    ...
    public String[] splitText(String text) {
        if (text.length() < 17) {
            ...
            String right = text.substring(16);
    ...
    public void setupScoreboard(Player player, String text, String name) {
        String[] splitText = this.splitText(text);

        Scoreboard scoreboard = player.getScoreboard() == null
                ? Bukkit.getScoreboardManager().getNewScoreboard()
                : player.getScoreboard();

        Team team = scoreboard.getTeam(name) == null
